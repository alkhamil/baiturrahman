...
class Pengeluaran_model extends CI_Model {
    ...
    public $column_order = ['t_pengeluaran.amount', 't_pengeluaran.code', 'm_trans_type.name']; //set column field database for datatable orderable
    public $column_search = ['t_pengeluaran.amount', 't_pengeluaran.code', 'm_trans_type.name']; //set column field database for datatable searchable 
    ...
    function lists($select = '*', $where = null, $limit = 10 ,$offset = 0)
    {
        $this->db->select($select)
                 ->join('m_trans_type', 'm_trans_type.id = t_pengeluaran.trans_type_id', 'left')
                 ->limit($limit,$offset);
    ...
    function list_count($where = null, $is_where = false) {
        $this->db->join('m_trans_type', 'm_trans_type.id = t_pengeluaran.trans_type_id', 'left');
        if($is_where) {
            if($where) {
                if(isset($where['q']) && $where['q'])
                    $this->db->like("LOWER(".$this->search_field.")", $where['q']);
