...
class Transaction_model extends CI_Model {
    ...
    public $table = 't_transaction';
    public $primary_key = 't_transaction.id';
    public $order_by = 't_transaction.id';
    public $order_type = 'DESC';
    public $search_field = 't_transaction.code';
    public $column_order = ['t_transaction.amount', 't_transaction.code', 'm_trans_type.name', 'm_zakat_type.name', 'm_jamaah.name']; //set column field database for datatable orderable
    public $column_search = ['t_transaction.amount', 't_transaction.code', 'm_trans_type.name', 'm_zakat_type.name', 'm_jamaah.name']; //set column field database for datatable searchable 
    ...
    function lists($select = '*', $where = null, $limit = 10 ,$offset = 0)
    {
        $this->db->select($select)
                 ->join('m_trans_type', 'm_trans_type.id = t_transaction.trans_type_id', 'left')
                 ->join('m_zakat_type', 'm_zakat_type.id = t_transaction.zakat_type_id', 'left')
                 ->join('m_jamaah', 'm_jamaah.id = t_transaction.jamaah_id', 'left')
                 ->limit($limit,$offset);
    ...
    function list_count($where = null, $is_where = false) {
        $this->db->join('m_trans_type', 'm_trans_type.id = t_transaction.trans_type_id', 'left')
                ->join('m_zakat_type', 'm_zakat_type.id = t_transaction.zakat_type_id', 'left')
                ->join('m_jamaah', 'm_jamaah.id = t_transaction.jamaah_id', 'left');
